...
import Kernel.External.Whatsapp.TataCommunications.API as API
...
sendMessageApi tataCommunicationsCfg req = do
  ...
  callAPI
    ...
    (callTataCommunicationsWhatsapp authHeader req)
    "sendMessageApi"
    API.tataCommunicationsWhatsappConnectAPI
    >>= checkTataCommunicationsWhatsappError tataCommunicationsCfg.url
  where
    callTataCommunicationsWhatsapp = ET.client API.tataCommunicationsWhatsappConnectAPI
...
tataCommunicationsWhatsappError = ExternalAPICallError (Just "DIGOENGAGE_OPT_API_ERROR")
