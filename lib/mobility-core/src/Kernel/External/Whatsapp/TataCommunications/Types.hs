...
import Data.Aeson
import GHC.Generics hiding (to) -- Hides `to` from `GHC.Generics`
import Kernel.Prelude hiding (error) -- Hides `error` from `Kernel.Prelude`

-- Top-level request type
data WhatsAppMessageReq = WhatsAppMessageReq
  { recipient :: Text, -- Renamed 'to' to 'recipient' to avoid conflict
    type_ :: Text,
    template :: Template
  }
...
data WhatsAppErrorResponse = WhatsAppErrorResponse
  { errDetails :: WhatsAppErrorDetails -- Renamed 'error' to 'errDetails' to avoid conflict
  }
  deriving (Show, Generic)
...
renameFields "recipient" = "to" -- Maps 'recipient' back to 'to' in JSON
renameFields "errDetails" = "error" -- Maps 'errDetails' back to 'error' in JSON
