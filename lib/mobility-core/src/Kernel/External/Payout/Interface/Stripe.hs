...
buildPayoutEventObject eventType stripeObject = case (eventType, stripeObject) of
  ...
  (PayoutWh.PayoutStripeWebhookCustomEvent eventName, _) -> pure $ Events.CustomEvent eventName -- Handle unknown event types regardless of object shape — forward compatibility
  (_, _) ->
    throwError $
      InvalidRequest $
        "Invalid object: "
