...
advancedDirectionsAPI url key origin destination mode intermediates isAvoidTolls computeAlternativeRoutes routingPreference = do
  ...
  callAPI url (advancedDirectionsClient key "routes.legs.*,routes.distanceMeters,routes.duration,routes.staticDuration,routes.viewport.*,routes.routeLabels.*" req) "advancedDirectionsAPI" (Proxy :: Proxy GoogleMapsAPI)
    >>= checkGoogleMapsError' url
