...
module Kernel.External.Verification.Morth.Config where
...
prepareMorthHttpManager :: Int -> HashMap.HashMap Text Http.ManagerSettings
prepareMorthHttpManager timeout =
  HashMap.singleton (fromString morthHttpManagerKey) $
    HttpTLS.tlsManagerSettings
      { Http.managerResponseTimeout = Http.responseTimeoutMicro (timeout * 1000)
      }

morthHttpManagerKey :: String
morthHttpManagerKey = "morth-http-manager"
