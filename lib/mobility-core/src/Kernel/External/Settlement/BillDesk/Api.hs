...
type RetrieveSettlementAPI =
  "pasettlements" :> "ve1_2" :> "settlements" :> "get"
...
type RetrieveSettlementDetailsAPI =
  "pasettlements" :> "ve1_2" :> "settlements" :> "v2" :> "getSettlementDetails"
    :> Header "BD-Traceid" Text
    :> Header "BD-Timestamp" Text
    :> ReqBody '[ApplicationJose] LBS.ByteString
    :> Post '[ApplicationJose] JoseResponse
...
-- ---------------------------------------------------------------------------
-- Dynamic JOSE API (no path segments — path comes from BaseUrl)
-- ---------------------------------------------------------------------------
...
dynamicJoseAPI :: Proxy DynamicJoseAPI
dynamicJoseAPI = Proxy
...
dynamicJoseClient ::
  ...
  LBS.ByteString ->
  ET.EulerClient JoseResponse
dynamicJoseClient = ET.client dynamicJoseAPI
