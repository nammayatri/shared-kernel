...
fetchBillDeskSettlementDetails config settlement = do
  ...
  let converted = catMaybes $ zipWith (convertRecord settlement) [1 :: Int ..] allRecords
...
buildRuntimeCfg config = do
  let clientIdPlain = config.clientId
      signingKeyPlain = config.signingKey
      encryptionKeyPlain = config.encryptionKey
      encryptionKeyIdPlain = config.encryptionKeyId
  pure
    RuntimeBillDeskCfg
...
convertRecord :: SettlementSummary -> Int -> SettlementRecord -> Maybe (Either Text PaymentSettlementReport)
convertRecord settlement _idx rec =
  (parseTxnType =<< rec.transaction_type) <&> \txnType' ->
