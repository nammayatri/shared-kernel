...
import Data.Time (TimeZone (..), defaultTimeLocale, formatTime, parseTimeM, utcToLocalTime)
...
fetchBillDeskSettlements config fromDate toDate pvNumber = do
  ...
  let fromDateText = formatDateForBillDesk <$> fromDate
      toDateText = formatDateForBillDesk <$> toDate
  settlements <- retrieveSettlements runtimeCfg fromDateText toDateText pvNumber
  pure $ map convertSettlementObjToSummary settlements
...
fetchBillDeskSettlementDetails config settlement = do
  runtimeCfg <- buildRuntimeCfg config
  allRecords <- retrieveAllSettlementDetails runtimeCfg settlement.pvNumber
  let converted = zipWith (convertRecord settlement) [1 :: Int ..] allRecords
      (errs, goods) = partitionEithers converted
  pure
    ParseResult
      { reports = goods,
...
retrieveSettlements cfg fromDate toDate pvNumber = do
  ...
  case A.eitherDecode respBytes of
    Right (resp :: RetrieveSettlementResp) -> pure resp.records
...
sendBillDeskRequest cfg baseUrl payload clientFn desc apiProxy = do
  ...
  logInfo $ "Sending BillDesk request: " <> signedBody
...
decryptAndCheckError cfg resp = do
  ...
  logInfo $ "Received BillDesk response: " <> respText
  case parseBillDeskResponse cfg._signingKey cfg._encryptionKey respText of
...
convertRecord :: SettlementSummary -> Int -> SettlementRecord -> Either Text PaymentSettlementReport
convertRecord settlement _idx rec =
  case parseTxnType =<< rec.transaction_type of
    Nothing -> Left $ "unsupported transaction type: " <> fromMaybe "none" rec.transaction_type
    Just txnType' ->
      let base = baseReport settlement rec txnType'
       in case txnType' of
            ORDER -> toSettlementReport base rec
            REFUND -> toRefundReport base rec
            CHARGEBACK -> toChargebackReport base rec

baseReport :: SettlementSummary -> SettlementRecord -> TxnType -> PaymentSettlementReport
...
toSettlementReport :: PaymentSettlementReport -> SettlementRecord -> Either Text PaymentSettlementReport
toSettlementReport base rec = do
  oid <- maybeToEither "settlement record missing merc_ref_id (orderid)" rec.merc_ref_id
  Right
    base
      { orderId = extractActualOrderId oid,
...
toRefundReport :: PaymentSettlementReport -> SettlementRecord -> Either Text PaymentSettlementReport
toRefundReport base rec = do
  oid <- maybeToEither "refund record missing merc_ref_id (refundorderid)" rec.merc_ref_id
  Right
    base
      { orderId = extractActualOrderId oid,
...
toChargebackReport :: PaymentSettlementReport -> SettlementRecord -> Either Text PaymentSettlementReport
...
convertSettlementObjToSummary :: SettlementObj -> SettlementSummary
convertSettlementObjToSummary obj =
  SettlementSummary
    { pvNumber = obj.pv_number,
      merchantId = obj.mercid,
      payoutMerchantId = obj.payout_mercid,
      pvFile = obj.pv_file,
      pvFileDate = obj.pv_file_date >>= parseDateText,
      currency = INR,
      amountDetails = convertAmountDetailsToSummary <$> obj.amount_details,
      charges = parseAmountMaybe' obj.charges,
      taxes = parseAmountMaybe' obj.taxes,
      otherAdjustments = parseAmountMaybe' obj.other_adjustments,
      payoutAmount = parseAmountMaybe' obj.payout_amount,
      status = obj.status,
      settlementDate = obj.settlement_date >>= parseDateText,
      utr = obj.utr,
      utrDate = obj.utr_date >>= parseDateText
    }

convertAmountDetailsToSummary :: AmountDetails -> AmountSummary
convertAmountDetailsToSummary ad =
  AmountSummary
    { settlement = parseAmountMaybe' ad.settlement,
      refund = parseAmountMaybe' ad.refund,
      chargeback = parseAmountMaybe' ad.chargeback,
      refundReversal = parseAmountMaybe' ad.refund_reversal,
      chargebackReversal = parseAmountMaybe' ad.chargeback_reversal,
      adjustment = parseAmountMaybe' ad.adjustment
    }

parseAmountMaybe' :: Maybe Text -> Maybe HighPrecMoney
parseAmountMaybe' = fmap parseAmount

parseTxnType :: Text -> Maybe TxnType
parseTxnType t = case T.toLower (T.strip t) of
  "settlement" -> Just ORDER
  "refund" -> Just REFUND
  "chargeback" -> Just CHARGEBACK
  _ -> Nothing

formatDateForBillDesk :: UTCTime -> Text
formatDateForBillDesk =
  let ist = TimeZone 330 False "IST"
   in T.pack . formatTime defaultTimeLocale "%Y%m%d" . utcToLocalTime ist
...
-- Extract actual order ID from PG format like "PREFIX-actualOrderId-SUFFIX"
extractActualOrderId :: Text -> Text
extractActualOrderId rawId =
  case T.splitOn "-" rawId of
    (_ : actualId : _) -> actualId
    _ -> rawId
