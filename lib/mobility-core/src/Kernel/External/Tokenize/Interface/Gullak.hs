...
gullakOnboarding config req = do
  ...
  where
    ...
    mkReqOnboardingReq InterfaceTypes.OnboardingReq {..} = GullakTypes.OnboardingReq {languageDetails = mkReqLanguageDetails <$> languageDetails, address = mkReqAddress <$> address, locationDetials = mkReqLocationDetials <$> locationDetials, ..}

    mkReqLanguageDetails :: InterfaceTypes.LanguageDetails -> GullakTypes.LanguageDetails
    mkReqLanguageDetails InterfaceTypes.LanguageDetails {..} = GullakTypes.LanguageDetails {..}

    mkReqLocationDetials :: InterfaceTypes.LocationDetials -> GullakTypes.LocationDetials
    mkReqLocationDetials InterfaceTypes.LocationDetials {..} = GullakTypes.LocationDetials {..}
