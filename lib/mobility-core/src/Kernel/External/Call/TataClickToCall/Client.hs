...
tataInitiateCall tataClickToCallCfg req = do
  ...
  callAPI tataClickToCallCfg.url (eulerClient (Just $ "Bearer " <> apiKey_) req) "tataInitiateCall" (Proxy @TataClickToCallAPI)
    >>= fromEitherM (ClickToCallGenericError . T.pack . show)
