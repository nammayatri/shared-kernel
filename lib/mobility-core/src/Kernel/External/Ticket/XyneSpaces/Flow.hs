...
-- | The read clients carry the app JWT as a bearer header; refuse to send
-- it in cleartext. Plain HTTP stays allowed for localhost so dev mocks
-- keep working.
ensureHttps :: MonadFlow m => BaseUrl -> m ()
ensureHttps url =
  when (SC.baseUrlScheme url == SC.Http && SC.baseUrlHost url `notElem` ["localhost", "127.0.0.1"]) $
    throwError $ InternalError "Refusing to send the Xyne app JWT over plain HTTP — configure an https:// base URL"
...
downloadFileAPI url token attachmentId = do
  ...
  if status >= 200 && status < 300
    then
      ...
      _ -> throwError $ InternalError $ "Xyne file download failed with status " <> show status <> " for attachment " <> attachmentId
  where
