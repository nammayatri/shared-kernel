...
createTransfer config req = do
  ...
  case config.chargeDestination of
    Platform -> mkCreateTransferResp <$> Stripe.createTransfer url apiKey Nothing transferReq
    ConnectedAccount -> mkCreateTransferResp <$> Stripe.createTransfer url apiKey (Just req.senderConnectedAccountId) transferReq
  where
