...
createOrder config req = do
  ...
  Juspay.createOrder url apiKey merchantId (mkCreateOrderReq config.returnUrl config.clientId req)
    ...
    Just justMandate ->
      MandateOrderStatusResp
        { orderShortId = order_id,
          ...
          mandateStartDate = posixSecondsToUTCTime (read $ T.unpack justMandate.start_date),
          mandateEndDate = posixSecondsToUTCTime (read $ T.unpack justMandate.end_date),
